...
import vertexShader from './shaders/trailtexture/vertex.glsl';
import fragmentShader from './shaders/trailtexture/fragment.glsl';
import TrailTextureGenerator from './useTrailTexture.js';
...
export default class TrailTexture {
  constructor() {
    ...
    if (this.debug.active) {
      this.debugFolder = this.debug.ui.addFolder('TrailTexture');
    }
    this.settings = {
      progress: 1,
    };
  ...
  raycast() {
    ...
    if (intersect.length > 0) {
      ...
      // console.log(intersect[0].uv.clone().sub(new Vector2(1, 1)).length());
  ...
  update() {
    ...
    // console.log(this.time.elapsed % 3);
  }
}
